...
void twophaseRowCol(const int myrow, const int mycol, const int rank, const int size, 
    ...
    if (mycol == 0) {
        ...
        for (int row=0; row<blocks[0]; row++) {
	    	...
	    	//printf("%d %d %d",senddispls[row], senddispls[row-1], sendcounts[row-1]);
	    }
        }
	...
	//if(rank == 2){printarray(rowdata, sendcounts[myrow], globalsizes[1]);}
    ...
    if (mycol == 0) {
	    ...
	    //printf("%d [%d] %d %d\n",rank,col,sendcounts[col],senddispls[col]);
        }
    }
