...
*/     
	...
	double *RA = calloc(m*k,sizeof(double));
	double *ans = calloc(m*k,sizeof(double));
	MPI_Gather(unit_A,blocksize_A,MPI_DOUBLE,RA,blocksize_A,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	

	if(rank == 0){
		
		reshape(RA,size,n,blocks,m/blocks[0],k/blocks[1],ans,2);
		print2d(ans,m,k);
	}
